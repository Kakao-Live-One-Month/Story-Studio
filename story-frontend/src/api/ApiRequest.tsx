...
export const generateOption = async () => {
  try {
    ...
    const apiResponse: string = await OpenAI(optionApiRequestPrompt);
    const splitResponse: string = apiResponse.split("\n").join('');
    const splitResponse2: string = splitResponse.split("\"").join('');
    console.log("옵션 반환 값: ", splitResponse2);
  
    const parsedData: selectionType = JSON.parse(splitResponse2);
